...
| `meilisearch_env_variables`                    | `{}`                                 | Dict of environment variables for the env file (file is created only when non-empty) |
...
- The environment file is only rendered when `meilisearch_env_variables` is non-empty.
